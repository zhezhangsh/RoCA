...
name.packages <- c('rmarkdown', 'knitr', 'kableExtra', 'yaml', 'DT', 'htmlwidgets', 'gplots', 'RoCA', 'awsomics', 'DEGandMore'); 
...
<div style="color:darkblue">
`r OrderFigure()` Clustering of outliers based on their difference with the non-outliers in the same group. If multiple outliers have similar global pattern of changes relative to their non-outliers, these outliers could be the consequence of the same systematic bias. With the cutoff of correlation coefficient equal to `r prms$cut.corr`, the following outliers are clustered together: 
...
<div style="color:darkblue">
`r OrderTable()` The numbers of top genes selected from each outlier.
</div>

```{r top_gene, include=TRUE}
if (nrow(outliers) == 0) 'No outliers found' else {
  # select DEGs
  ...
  kable(tbl, row.names=TRUE, align=c('c', 'c')) %>% kable_styling(
...
<div style="color:darkblue">
`r OrderTable()` The number of gene sets significantly enriched in top increasing genes of each outlier.
</div>

```{r gsea_hi, include=TRUE}
if (nrow(outliers) == 0) 'No outliers found' else {
  kable(tbl.hi, align = rep('c', ncol(tbl.hi))) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)
}
```
...
<div style="color:darkblue">
`r OrderTable()` The number of gene sets significantly enriched in top decreasing genes of each outlier.
</div>

```{r gsea_lo, include=TRUE}
if (nrow(outliers) == 0) lns <- 'No outliers found' else {
  lns <- kable(tbl.lo, align = rep('c', ncol(tbl.lo))) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)
}
```
