...
if (nrow(outliers) == 0) lns <- 'No outliers found' else {
  ...
  lns <- kable(tbl, row.names=TRUE, align=c('c', 'c')) %>% kable_styling(
...
```{r gsea_hi, include=FALSE}
if (nrow(outliers) == 0) lns <- 'No outliers found' else {
  lns <- kable(tbl.hi, align = rep('c', ncol(tbl.hi))) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)
}
```
