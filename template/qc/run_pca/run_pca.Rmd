...
`r kable(FormatNumeric(tbl)) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), position='left', full_width=FALSE)`
...
```{r remove_pc}
res <- lapply(1:min(16, ncol(cov)), function(i) {
  x <- cov[, i];
...
names(res) <- paste('PC', 1:min(16, length(res)));
