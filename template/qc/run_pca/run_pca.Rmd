...
if (!is.null(fil.sd) & length(fil.sd)>0) if (!is.na(fil.sd)) {
  nsd <- sapply(1:ncol(mtrx), function(i) abs(mtrx[, i]/apply(mtrx[, -i, drop=FALSE], 1, sd)));
...
smp <- smpl[, !(colnames(smpl) %in% names(prms$exclude)), drop=FALSE];
...
fns <- sapply(colnames(smp), function(nm) { 
  ...
  png(fn, w=800, h=600); 
  cex <- min(4, max(0.5, 256/nrow(smp))); 
  ...
  PlotPCA(pca, groups=rownames(smp), new.window = FALSE, cex=cex, col=col, label=1:nrow(smp), 
