...
if (!exists('yml'))                               # if the 'yml' variable doesn't exist yet, create it by loading the YAML file
  if (file.exists('filter_read.yaml'))           # assume the pairing YAML file exists in the current folder with the same name
      yml<-yaml.load_file('filter_read.yaml');   
...
smpl<-DownloadFile(yml$input$sample, path.input);
...
summarizeANDfilter<-function(s, yml, rngs) {
  ...
  gr<-ImportR(DownloadFile(fn, paste(yml$output, 'input', sep='/')));
