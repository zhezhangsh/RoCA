...
- Average number of total reads per gene: ***`r format(mean(count.sum2[, 1]), big.mark=',')`***
- Average number of total reads per sample: ***`r format(mean(count.sum1[, 1]), big.mark=',')`***
...
- **`r pct1`%** genes have 1 or less read mapped to them per sample.
- **`r pct5`%** genes have 5 or less reads mapped to them per sample.
...
ids10 <- apply(ct, 2, function(x) rownames(ct)[rev(order(x))][1:10]);
...
This section uses read count data to perform several sample-level analyses. The results can be used to identify potential mislabeling, outlier, confounding variable, etc. Read counts normalized by the _Rlog_ method are used for all analyses in this section. 
...
#         msg <- 'Samples cannot be classified into 2 groups by neither X nor Y genes.'
  km.xy <- pamk(cbind(colMeans(ct.x), colMeans(ct.y)), krange=1:min(5, ncol(ct.x)));
  nc.xy <- km.xy$nc;
...
plot(hclust(as.dist(1-cor(e))), xlab='', sub='', ylab='1 - correlation coefficient', 
     main='Hierarchical clustering using all autosomal genes', cex.lab=1.25);
```
