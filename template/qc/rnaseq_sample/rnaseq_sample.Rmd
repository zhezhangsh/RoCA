...
- Average number of reads per gene: _`r format(mean(count.sum2[, 1]), big.mark=',', digit=0)`_
- Total number of reads per sample: _`r format(mean(count.sum1[, 1]), big.mark=',')`_
...
<div style="color:darkblue; padding:0 1cm">
**Figure 4.** Total mapped reads of all samples are split by mapping types, such as unique vs. multiple, paired vs. unpaired, and sense vs. antisense. Among the `r length(count.all)` mapping type(s), 
...
fn <- CreateDatatable(round(corr, 4), paste(path.tbl, 'corr_mapping_type.html', sep='/'), caption='Correlation of read counts between mapping types'); 
...
r <- corr[-1, 1];
nm <- names(r)[r==max(r)];
PlotLogScatter(count.sum2[, 1], count.sum2[, nm], xlab=colnames(count.sum2)[1], ylab=nm, main = paste('R =', round(corr[1, nm], 4))); 
abline(0, 1, lwd=1, col='red'); 
} else {
...
<div align='center'>
```{r normalization_boxplot, fig.width=10.5, fig.heigth=3.6, out.width='900px'}
col <- gray.colors(length(unique(smpl[, 1]))+1)[-1]; 
col <- col[as.integer(factor(smpl[, 1]))]; 
par(mfrow=c(2, 5), omi=c(0, 0.5, 0, 0), mai=c(0, 0, .5, 0)); 
for (i in 1:10) boxplot(data.frame(log2(norm[[i]][ind, ]+1)), outline=FALSE, boxwex =1, staplewex=0, outwex=0, 
...
**Figure 8.** Each panel corresponds to a normalization method, including the boxplots of all samples. Samples are colored by their **`r names(smpl)[1]`**.
</div>
...
tbl <- data.frame(Method=colnames(cv), Mean=colMeans(cv), Corr2Original=cor(cv, cv)[, 1], Num_Decrease=c[, 1],
                  Num_Increase=c[, 2], "Decrease_vs_Increase" = c[, 1]/c[, 2]);  
```
</div>
