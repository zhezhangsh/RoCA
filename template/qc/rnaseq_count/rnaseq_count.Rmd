...
plot(mn, cv, pch=19, cex=.5, xaxt='n', cex.lab=1.5, col='#88888888', xaxs='i', yaxs='i', xlim=c(0, max(mn)),
     xlab='Average read count of all samples', ylab='Coefficient of variation', ylim=c(0, min(max(cv, na.rm=TRUE), 6))); 
...
abline(0, 1);
PlotLogScatter(cnt[, p2[1]], cnt[, p2[2]], xlab=p2[1], ylab=p2[2], col="#1ABC9C",
...
ngh <- sapply(rownames(smm), function(id) {
  ...
  PlotLogScatter(cnt[, id], cnt[, mn], xlab=id, ylab=mn, col="#1ABC9C",
