...
name.packages <- c('rmarkdown', 'knitr', 'kableExtra', 'yaml', 'DT', 'htmlwidgets', 'arrangements', 
...
compareFreq <- function(seq, len) {
  ...
  cmb <- apply(arrangements::permutations(bas, k=len, replace=TRUE), 1, function(x) paste(x, collapse=''));
  exp <- 2^(rowSums(log2(arrangements::permutations(prb, k=len, replace=TRUE)))); # expected frequency
