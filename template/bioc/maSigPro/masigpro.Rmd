---
title: "Clustering of time series data in multiple conditions using _maSigPro_"
...
name.packages <- c('rmarkdown', 'knitr', 'yaml', 'DT', 'htmlwidgets', 
                   'GenomicRanges', 'RoCA', 'awsomics', 'maSigPro'); 
...
ts <- maSigPro(mtrx, dsgn, degree = gdr, rsq = prms$rsq, k = ncl, Q = prms$Q,
               alfa = prms$alfa, cluster.method = prms$cluster.method,
               step.method = prms$step.method, vars = 'all', pdf = FALSE);
saveRDS(ts, paste(path.r, 'all_result.rds', sep='/'));
...
# prof <- ts$sig.genes$Placebo$sig.profiles;
...
se <- lapply(1:length(cls), function(i) 
  sapply(grps, function(g) sapply(g, function(g) sd(mns[g, i])/length(g))));
mn <- lapply(mn, t);
se <- lapply(se, t);
...
m <- lapply(grps, function(g) sapply(g, function(g) colMeans(mns[g, , drop=FALSE])));
g <- lapply(m, colnames);
m <- do.call('cbind', m); 
x <- ceiling(max(abs(m))); 
...
CreateDatatable(FormatNumeric(m), paste(path.tbl, 'cluster_mean.rds', sep='/')) -> x; 
```
</div>
...
> [Click](table/cluster_mean.rds) to view full table of group means.
</div>
...
for (i in 1:length(cls)) {
  v <- se[[i]]; 
  v[is.na(v)] <- 0; 
  PlotSeries(mn[[i]], v, col=col, lty=lty, title=names(cls)[i], draw.legend = i==1);
}; 
```
</div>

<div align='center', style="color:darkblue; padding:0 0cm">
...
pv1 <- lapply(cls, function(c) {
  ps <- sapply(grps, function(g) {
    sapply(2:length(g), function(i)
      if (length(g[[1]])>1 & length(g[[i]])>1) {
        t.test(colMeans(mtrx[c, g[[1]]]), colMeans(mtrx[c, g[[i]]]), paired=prd)$p.value; 
      } else {
        t.test(rowMeans(mtrx[c, g[[1]], drop=FALSE]), 
               rowMeans(mtrx[c, g[[i]], drop=FALSE]), paired=TRUE)$p.value; 
      }
    )
  });
...
fns <- sapply(names(cls), function(nm) {  print(nm); 
  fn<-paste(path.cl, '/', nm, '.pdf', sep=''); 
  pdf(fn, w=6, h=12); 
  par(mfrow=c(4, 1), omi=c(0.2, 0.2, 0.2, 0.2));

  x <- mtrx[cls[[nm]], , drop=FALSE];
  x <- lapply(grps, function(g) sapply(g, function(g) rowMeans(x[, g, drop=FALSE])));
  y <- do.call('cbind', x); 
  ...
  PlotSeries(mn[[nm]], v, labs=c('', 'Group mean'), title=nm, 
...
**Table 1** The post-hoc analysis of clusters shown in **Figure 3**. The _between_ statistics represent the maximal deviation (when and how much) between a time point and the beginning time point, and the _within_ statistics represent the maximal deviation (when and how much) of the conditions from each other within the same time point.
</div>
