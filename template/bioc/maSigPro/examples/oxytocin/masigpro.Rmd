...
mtrx <- ImportTable(DownloadFile(yml$input$mtrx, path.input));
dsgn <- ImportTable(DownloadFile(yml$input$dsgn, path.input));
...
if (is.null(yml$input$anno)) anno <- NULL else {
  anno <- ImportTable(DownloadFile(yml$input$anno, path.input)); 
  anno <- anno[rownames(mtrx), , drop=FALSE];
  rownames(anno) <- rownames(mtrx); 
};

if (is.null(yml$input$geneset)) gset <- NULL else gset <- ImportList(DownloadFile(yml$input$geneset, path.input)); 
