...
dsgn <- ImportTable(DownloadFile(yml$input$dsgn));
...
if (is.null(yml$input$anno)) anno <- NULL else {
  anno <- ImportTable(DownloadFile(yml$input$anno)); 
  anno <- anno[rownames(mtrx), , drop=FALSE];
  rownames(anno) <- rownames(mtrx); 
};

if (is.null(yml$input$geneset)) gset <- NULL else gset <- ImportList(DownloadFile(yml$input$geneset)); 
