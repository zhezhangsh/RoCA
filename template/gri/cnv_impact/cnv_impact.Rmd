...
name.yaml <- 'cnv_impact.yaml'; 
name.packages <- c('RCurl', 'yaml', 'rmarkdown', 'knitr', 'gplots', 'vioplot', 'DT', 'htmlwidgets', 'Matrix', 
...
write.table(cnv, paste(path.tbl, '/', prms$feature, '_', 'cnv.txt', sep=''), quote=FALSE, col=TRUE, row=TRUE, sep='\t'); 
```
...
Summary of CNV-to-`r prms$feature` mapping by `r prms$feature`. 
  ...
  - [Full sample-`r prms$feature` matrix of copy numbers](`r as.vector(TruncatePathPrefix(fn2, path))`) 
...
ms <- sapply(stats, function(s) {
  x <- s[s[,1]>0.5 & s[,1]<=3.5 & !is.na(s[, 5]) & !is.na(s[, 4]), , drop=FALSE]; 
...
abline(v=1:3);
if (length(x[[1]])==0) vioplot(x[[2]], x[[3]], names=colnames(ms), col='gold', rectCol = 'darkgrey', at=2:3, add=TRUE) else
  if (length(x[[2]]==0)) vioplot(x[[1]], x[[3]], names=colnames(ms), col='gold', rectCol = 'darkgrey', at=c(1, 3), add=TRUE) else
    if (length(x[[3]])==0) vioplot(x[[1]], x[[2]], names=colnames(ms), col='gold', rectCol = 'darkgrey', at=1:2, add=TRUE) else
...
s<-lapply(names(cnv2fea), function(nm) {
  c2f <- cnv2fea[[nm]]; 
  s <- stats[[nm]]; 
  q1.0 <- summary(s[, 5])[2];
  q1.1 <- summary(s[, 2])[2];
...
names(cnt.all) <- c('original', 'adjusted_global', 'adjusted_by_sample', 'adjusted_by_cnv'); 
...
stats.all <- list(corr=crs, mean=mns, sd=sds, cv=cvs); 
saveRDS(stats.all, paste(path.r, 'adjusted_stat.rds', sep='/')); 
saveRDS(cnt.all, paste(path.r, 'adjusted_data.rds', sep='/')); 

write.table(cnt.all[[2]], paste(path.tbl, 'adjusted_globolly.csv', sep='/'), sep='\t', quote=FALSE, col=TRUE, row=TRUE); 
write.table(cnt.all[[3]], paste(path.tbl, 'adjusted_by_sample.csv', sep='/'), sep='\t', quote=FALSE, col=TRUE, row=TRUE); 
write.table(cnt.all[[4]], paste(path.tbl, 'adjusted_by_cnv.csv', sep='/'), sep='\t', quote=FALSE, col=TRUE, row=TRUE); 
```
...
In most cases, these adjustment methods, by their order, have increased specificity and are expected to reduce overall variance more and more, but also have increasing risk of overfitting. Whether to use adjusted data for statistical analysis or which adjustment method to choose should be determined by the nature of the study, the goal of the analysis, and the effect of the adjustment. In this data set, these methods reduce the overall variance by `r min(cvs.pct)` to `r max(cvs.pct)` percent on average.
  ...
  - [The `r prms$feature`-level coefficient of variation (CV) before and after adjustment](table/CV_adjustment.html)
