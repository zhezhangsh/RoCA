...
saveRDS(cns, paste(path.r, TrimPath(yml$input$cnv), sep='/')); 
saveRDS(reg, paste(path.r, TrimPath(yml$input$region), sep='/')); 
saveRDS(cnt, paste(path.r, TrimPath(yml$input$count), sep='/')); 
if (!identical(NA, ann)) saveRDS(ann, paste(path.r, TrimPath(yml$input$annotation), sep='/')); 
```
...
Summary of CNV-to-`r prms$feature` mapping by CNV. 
  ...
  - **`r round(weighted.mean(stat.cnv[, 3], w=stat.cnv[, 'Num_CNV']), 3)`**% of the CNVs are overlapped to any `r prms$feature`.
...
Using heterozygous deletion/duplication, the average percent change of `r prms$feature` `r prms$measurement` in response to the loss/gain of one copy is calculated for all samples globally and each sample individually. 

  - [CNV vs. diploid](table/CNV_vs_Diploid.html)
...
CreateDatatable(t, paste(path.tbl, 'average_change_by_sample.html', sep='/'), caption='Average effect of individual CNVs'); 
```
...
for (i in 1:nrow(inds)) {
  x <- cvs[, inds[i, ]]; 
  x <- x[!is.na(x[, 1]) & !is.na(x[, 2]), , drop=FALSE]
  t[inds[i, 1], inds[i, 2]] <- round(100*nrow(x[x[, 1]>1.05*x[, 2], , drop=FALSE])/nrow(x), 3);
  t[inds[i, 2], inds[i, 1]] <- round(100*nrow(x[x[, 1]<0.95*x[, 2], , drop=FALSE])/nrow(x), 3);
}
dimnames(t) <- list(paste(colnames(tbl)[1:4], '>'), paste('>', colnames(tbl)[1:4])); 
```

<div style="color:darkblue; padding:0 1cm">
**Table 1** The percentage of `r prms$feature`s whose coefficient of variation is relatively decreased (up-right corner) or increased (bottom-left corner) by 5% or more after the adjustment.
</div>
...
Check out the [RoCA project](http://zhezhangsh.github.io/RoCA) to read more.  
