...
res <- DESeq2::results(dds, contrast = c(cnd[1], g0, g1));

pvl <- 2*pnorm(-abs(res[, 'stat'])); 
qvl <- res[, 'padj'];
fc0 <- res[, 'log2FoldChange'];
fc1 <- DESeq2::lfcShrink(dds, coef = resultsNames(dds)[2], type = 'normal')[, 2];
pvl[is.na(pvl)] <- 1;
qvl[is.na(qvl)] <- 1;
fc0[is.na(fc0)] <- 0;
fc1[is.na(fc1)] <- 0;
...
m <- rowMeans(cnt2[, c(s0, s1), drop=FALSE]);
d <- density(log2(m+1));
x <- d$x;
y <- d$y;
...
x0  <- as.vector(log2(1+summary(m)))[2:5];
...
text(x0, y0/2, srt=90, labels=round(2^x0-1, 1), cex=2);
l <- c('First quantile', 'Median', 'Mean', 'Third quantile');
...
# WriteExcel(tbl, paste0(path.tbl, '/pca'));
```
...
x <- log10(x);
dens <- density(x);
xlim <- c(0, dens$x[max(which(dens$y>=0.001))]);
ylim <- c(-max(y), max(y));
...
```{r deg_no1, include=TRUE, fig.width=8, fig.height=3, out.width='800px'}
# Plot top genes
...
col <- col1[c(s0, s1)];
for (i in 1:2) {
  barplot(cnt2[ind[i], c(s0, s1)], las=3, cex.names=cex, main=ttl[ind[i]], cex.main=1, cex.axis=.75, col=col, 
...
<div style="color:darkblue">
`r OrderFigure()` The top-ranked gene with the most up- (left) and down- (right) regulated expression in `r g1`. 
</div>
  
```{r deg_heatmap, include=TRUE, fig.width=6.4, fig.height=6.4, out.width='640px'}
# Plot heatmap of top genes
par(mfrow=c(1,1));
goi <- log2(cnt2+1)[c(rownames(up), rownames(dn)), c(s0, s1), drop=FALSE];
...
deg <- list(sample=smpl, anno=anno, parameter=prms, count=list(original=cnt1, normalized=cnt2), pca=pca);
deg$stat <- stat;
