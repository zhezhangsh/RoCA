...
cnt1 <- cnt1[, c(s0, s1), drop=FALSE];
smpl <- smpl[c(s0, s1), , drop=FALSE];
...
if (length(cnd) > 1) {
  adj0 <- rlg0;
  var0 <- apply(adj0, 1, sd);
  for (i in 2:length(cnd)) 
    adj0[var0>0, ] <- ComBat(adj0[var0>0, , drop=FALSE], as.vector(smpl[, cnd[i]]));
...
if (length(cnd) == 1) ln <- '' else {
  ...
  pdf(paste0(path.fig, '/hier_clustering_adjusted-1.pdf'), h=9.6, w=9.6);
  heatmap.2(corr, key=TRUE, key.title='Correlation coefficient', keysize=1, key.xlab='', key.ylab='',
          trace='none', tracecol=NULL, margins=c(10, 10), col=rev(heat.colors(101)));
  try(dev.off());
  ln <- paste0('  - [Clustering analysis](figure/hier_clustering_adjusted-1.pdf) after adjusting data for ***', 
...
title(xlab = xlab, ylab = ylab, line = 1.8, cex.lab = 1);
...
plot(x, y, main=title, pch=19, col='#CB4335',  cex=0.8*cx, 
     xlab='', ylab='', cex.axis=.75, ylim=c(0, 0.1+ylim), yaxs='i', xlim=xlim, yaxt='n');
title(xlab=xlab, ylab='P value', line=1.8, cex.lab=1);
axis(2, at=0:9, labels=10^(-1*(0:9)), las=2, cex.axis=0.5);
axis(2, at=10, labels="<= 10e-10", las=2, cex.axis=0.5);
...
title(xlab='P value', ylab='Count', line=1.8, cex.lab=1);
box();
...
title(xlab='FDR cutoff', ylab='Count', line=1.8, cex.lab=1);
box();
```
