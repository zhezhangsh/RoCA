...
saveRDS(expr, file=paste(path.input, 'expr.rds', sep='/')); 
saveRDS(anno, file=paste(path.input, 'anno.rds', sep='/')); 
saveRDS(smpl, file=paste(path.input, 'sample.rds', sep='/')); 
saveRDS(gset, file=paste(path.input, 'geneset.rds', sep='/')); 
```
...
<div align='center'>
```{r anova_pvalue, fig.width=8, fig.height=6, out.width='640'}
par(mar=c(5,5,2,2));
...
<div style="color:darkblue; padding:0 3cm">
**Figure 2.** Distribution of ANOVA p values. `r length(p[p<prms$selection$p])` genes have p values less than `r prms$selection$p`.
</div>
...
plot(hclust(as.dist(1-cor(expr0))), main='All genes', xlab='', sub='', ylab='');
plot(hclust(as.dist(1-cor(expr0[rownames(e), , drop=FALSE]))), main='Selected DEGs', xlab='', sub='', ylab='');
```
</div>
...
ms<-sapply(names(cl), function(nm) { 
  ...
  if (prms$plot$rescale) mtrx<-t(scale(t(mtrx)));
  if (prms$plot$zero) mtrx<-apply(mtrx, 2, function(x) x-rowMeans(mtrx[, grp[[1]], drop=FALSE]));
  x<-sortColumn(x, grp);
...
if (prms$plot$zero) d<-apply(d, 2, function(x) x-rowMeans(d[, grp[[1]], drop=FALSE]));
