...
name.packages <- c('rmarkdown', 'knitr', 'yaml', 'DT', 'htmlwidgets', 'MASS', 'gplots', 'pathview', 'PGSEA', 
                   'xlsxjars', 'xlsx', 'flexclust', 'colorspace', 'RoCA', 'rchive', 'awsomics', 'DEGandMore'); 
...
yml$gsea$gmt <- lapply(yml$input$gmt, function(x) DownloadFile(x, path.input)); 

fn.xls <- paste(g1.name, '-vs-', g2.name, '.xlsx', sep='');
```
...
Click links below to go directly to results or read the rest of this report for details: 
  ...
  - Download
    - [Result tables in Excel](table/`r fn.xls`)
    - [Figures in PDF](figure/figures.zip)
</div>
...
This report is generated based on the following inputs:
  ...
  - **Gene set analysis:**
    ...
    - **KEGG pathway maps:** `r yml$input$kegg` (`r length(kegg.fns)` total maps)
...
<div style="color:darkblue; padding:0 2cm">
  **Table 1.** Number of top DEGs selected via different cutoffs of FDR. FDRs are calculated using the Benjamini&Hochberg method (_Controlling the false discovery rate: a practical and powerful approach to multiple testing_. Journal of the Royal Statistical Society Series B 57, 289–300. 1995)
</div>
...
# PDF figures
fn.pdf <- paste(path.fig, dir(path.fig), sep='/'); 
fn.pdf <- fn.pdf[grep('.pdf$', fn.pdf)];
zip(paste(path.fig, 'figures.zip', sep='/'), fn.pdf); 
