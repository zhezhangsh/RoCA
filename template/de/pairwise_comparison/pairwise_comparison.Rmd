...
select.deg <- function(out, n, cutoffs) {
  de0 <- out[order(out[, 'Pvalue']), , drop=FALSE];
  ind <- which(de0[, 'Pvalue']<=cutoffs[1]);
  if (max(ind) >= n) de0 <- de0[1:max(ind), , drop=FALSE] else de0 <- de0[1:n, , drop=FALSE];
  if (nrow(de0) > n) {
    de1 <- de0[order(de0[, 'FDR']), , drop=FALSE];
    ind <- which(de1[, 'FDR']<=cutoffs[2]);
    if (max(ind) >= 0) de0 <- de1[1:max(ind), , drop=FALSE] else de0 <- de1[1:n, , drop=FALSE];
  };
  if (nrow(de0) > n) {
    de1 <- de0[rev(order(abs(de0[, 'LogFC']))), , drop=FALSE];
    ind <- which(abs(de1[, 'LogFC'])>=cutoffs[3]);
    if (max(ind) >= 0) de0 <- de1[1:max(ind), , drop=FALSE] else de0 <- de1[1:n, , drop=FALSE];
  };
  de0;
};
...
######################## NEED REVISION #############################
  ...
  degs <- lapply(out[1:2], function(x) select.deg(x, num.top, c(cutoff.p, cutoff.fdr, cutoff.l2r)));
  # degs <- lapply(out[1:2], function(x) {
  #   deg <- x[x[, 'Pvalue']<=cutoff.p, , drop=FALSE];
  #   if (nrow(deg) >= num.top) deg <- deg[abs(deg[, 'FDR']) <= cutoff.fdr, , drop=FALSE];
  #   if (nrow(deg) >= num.top) deg <- deg[abs(deg[, 'LogFC']) >= cutoff.l2r, , drop=FALSE];
  #   deg;
  # });
...
######################## NEED REVISION #############################
  ...
  out <- de$results$stat;
  deg <- select.deg(out, num.top, c(cutoff.p, cutoff.fdr, cutoff.l2r))
  # deg <- out[out[, 'Pvalue'] <= cutoff.p, , drop=FALSE];
  # if (nrow(deg) >= num.top) deg <- deg[deg[, 'FDR'] <= cutoff.fdr, , drop=FALSE]; 
  # if (nrow(deg) >= num.top) deg <- deg[abs(deg[, 'LogFC']) >= cutoff.l2r, , drop=FALSE]; 
  deg  <- deg[order(-1*abs(deg[, 'LogFC'])), , drop=FALSE];
  degs <- list(deg[deg[, 'LogFC']>0, , drop=FALSE], deg[deg[, 'LogFC']<0, , drop=FALSE]); 
}
...
This report is generated based on the following inputs:
  ...
  - **Comparison:** 
    ...
    - **Group1, `r g2.name`:** `r paste(colnames(e2), collapse='; ')` (n=`r ncol(e2)`)
    - **Paired:** `r prms$paired`
  - **Differential gene expression:** 
    - **Statistical method:** `r sub('^De', '', prms$deg$method)`
    - **Rank gene:** `r l1`
    - **Use reprocessed matrix:** `r prms$deg$reprocessed`
  - **Select top genes:**
...
boxplot(data.frame(e1.2), las=3, notch=TRUE, boxwex=.6, 
        col=c(rep('lightgrey', ncol(e1)), rep('lightblue', ncol(e2))), 
        names=colnames(e1.2), cex.axis=min(1, 0.4/cex), yaxt='n', add=TRUE, pch=19, cex=0.4) -> x;
```
...
hist(p[p<1], br=seq(0, 1, 0.01), cex.axis=.75, main='C. P Value Distribution', col='#D0D3D4');
...
lines(seq(min(q), 1, 0.01), n, lwd=2, col='#E74C3C');
...
<div style="color:darkblue">
`r OrderTable()` Number of top DEGs selected via different cutoffs of FDR. FDRs are calculated using the Benjamini&Hochberg method (_Controlling the false discovery rate: a practical and powerful approach to multiple testing_. Journal of the Royal Statistical Society Series B 57, 289–300. 1995)
</div>
...
if (ind > 0) up.tbl <- cbind('Entrez_ID'=up.tbl[, ind], ID=rownames(up.tbl), up.tbl[, -ind]);
up.formatted     <- GeneList2Datatable(FormatNumeric(up.tbl), fn=paste(path.deg1, 'index.html', sep='/'), 
                                       col.entrez=as.integer(prms$entrez>0), genome=genome, 
...
if (ind > 0) dn.tbl <- cbind('Entrez_ID'=dn.tbl[, ind], ID=rownames(dn.tbl), dn.tbl[, -ind]);
...
for (ii in 1:2) barplot(e1.2[ind[ii], ], las=3, cex.names=cex, main=CleanHtmlTags(anno[ind[ii], 1], FALSE), 
...
<div style="color:darkblue">
`r OrderTable()` Summary of ORA results. Numbers indicate the significant gene sets (p < 0.05) enriched in the DEGs with higher and lower expression in `r g2.name`. Click the numbers to view the full statistical results of individual gene sets.
</div>

`r kable(t, align=c('l', 'l', 'c', 'c', 'c')) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)`
  
```{r ora_ex_venn, include=TRUE, fig.width=4.8, fig.height=3.6, out.width='480px'}
...
<div style="color:darkblue">
`r OrderFigure()` Example: One of the top gene sets over-represented in genes with differential expression in `r g2.name`.
</div>
...
`r kable(t, align=c('l', 'c', 'c', 'c')) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)`
...
<div style="color:darkblue">
`r OrderFigure()` Example: The GSEA enrichment plot of the most significant gene set. The detailed intepretation of enrichment plot can be found [here](http://software.broadinstitute.org/gsea/doc/GSEAUserGuideTEXT.htm#_GSEA_Statistics). 
</div>
...
if (nrow(kegg.stat) == 0) {
...
} else if (prms$entrez < 0) {
  l <- "Entrez gene ID not provided!";
  kegg.ind <- kegg.stat[, -1];
} else {
  ...
  ind <- prms$entrez;
  if (ind > 0) names(kegg.dff) <- anno[names(kegg.dff), ind];
  kegg.dff <- kegg.dff[names(kegg.dff)!='' & !is.na(names(kegg.dff))];
  
  fn.kegg<-sapply(kegg.id, function(id) {
    try({
      capture.output(
        kegg <- pathview(
          kegg.dff, pathway.id=id, species = GetGenomeAlias(genome, "code"), 
...
<div style="color:darkblue">
`r OrderTable()` Gene-gene set biclusters identified from results of both ORA and GSEA. Click links to view lists of biclusters.
</div>

`r kable(t, align=c('c', 'c')) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width=FALSE)`
...
<div style="color:darkblue">
`r OrderFigure()` Example: a gene-gene set biclustering module. Blue means the gene is both a DEG and a member of the gene set.
</div>
