...
This procedure evaluates the efficiency of primers for targeted sequencing, in term of
...
General characteristics of all primers. Click link below to view full table.
...
An ideal primer should only have perfect match to the targeted regions in the whole reference genome. In addition, they should have as few matches to the off-target regions as possible even when mismatches are allowed. Click link below to view full table of match locations:
...
saveRDS(tbl, paste(path.r, 'match.rds', sep='/'));
...
para.match <- function(x) {
  require(Biostrings);
  vmatchPattern(x[[1]], x[[2]], x[[3]], x[[4]]);
};

seq1 <- seqns[nm1>=1&nm0==0];

if (TRUE) {  #################################### FOR DEBUG ONLY ####################################
mis <- lapply(1:max(1, max(1, prms$match$mismatch)), function(mis) {
  if (prms$match$multicore <= 1) {
    c <- lapply(seq1, function(s) { print(s); 
      ...
      if (prms$match$strand == 1) summ.match(vmatchPattern(s, genom, mis, mis), 1) else
        if (prms$match$strand == -1) summ.match(vmatchPattern(reverseComplement(s), genom, mis, mis), -1) else
          rbind(summ.match(vmatchPattern(s, genom, mis, mis), 1), 
               summ.match(vmatchPattern(reverseComplement(s), genom, mis, mis), -1));
    }); 
  } else {
...
<div style="color:darkblue">
`r OrderFigure()` Number of hits in the reference genome matching each primer sequence, while allowing up to `r prms$match$mismatch` mismatches. Locations of hits are split according to whether they are on or off targeted regions. Color represents number of mismatched bases. Click [here](table/mismathc_count.html) to view the full table of hit counts and [here](table/mismatch_location.html) to view the full table of hit locations. 
</div>
...
General characteristics of sequencing reads.
...
aln <- lapply(0:max(1, max(1, prms$match$mismatch)), function(mis) { print(mis);
  if (prms$match$multicore <= 1) {
    c <- lapply(seq1, function(s) { print(s); 
      # sum(vmatchPattern(s, genom, mis, mis)) + sum(vmatchPattern(reverseComplement(s), genom, mis, mis));
      if (prms$match$strand == 1) summ.match(vmatchPattern(s, reads, mis, mis), 1) else
        if (prms$match$strand == -1) summ.match(vmatchPattern(reverseComplement(s), reads, mis, mis), -1) else
          rbind(summ.match(vmatchPattern(s, reads, mis, mis), 1), 
               summ.match(vmatchPattern(reverseComplement(s), reads, mis, mis), -1));
    }); 
  } else {
    cl <- makeCluster(prms$match$multicore, type='SOCK');
    if (prms$match$strand == 1) {
...
saveRDS(aln, paste(path.r, 'alignment_all.rds', sep='/'));
```

### Primer summary

Summary of primer-to-read alignment by primers.
...
barplot(t(cnt[, ncol(cnt):1, drop=FALSE]), col=col, border=NA, space=0.2, 
        ylim=c(0, 1.15*max(rowSums(cnt))), ylab='Number of hits', cex.lab=1.25, 
...
saveRDS(cnt, paste(path.r, 'align_primer.rds', sep='/'));
CreateDatatable(tbl, paste(path.tbl, 'align_primer', sep='/')) -> x;
```

<div style="color:darkblue">
`r OrderFigure()` Occurance of primer sequences in sequencing reads. Color represents number of mismatched bases. Click [here](align_primer.html) to view full table of hit counts.
</div>

### Read summary

Summary of primer-to-read alignment by sequencing reads.
...
<div style="color:darkblue">
`r OrderFigure()` Number of matches to primer sequences per sequencing reads. 
</div>
