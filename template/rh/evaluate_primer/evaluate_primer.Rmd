...
run.match2 <- function(s, g, mx, mn, indel, strand=NA) {
  if (indel) { print(mx);
    ...
    smm <- data.frame(seqnames=rep(names(mth), elementNROWS(mth)), 
...
<div style="color:darkblue">
`r OrderFigure()` Occurance of primer sequences in sequencing reads. Color represents number of mismatched bases. Click [here](alignment_count_primer.html) to view full table of hit counts.
</div>
...
<div style="color:darkblue">
`r OrderFigure()` Number of reads including up to `r nrow(cnt)` matches to primer sequences. Color represents number of mismatches. Click [here](alignment_summary_read.html) to view full table of read counts.
</div>
