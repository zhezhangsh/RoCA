...
`r OrderTable()` Input data (primers, sequencing reads, targeted regions, and reference genome). (_Genome_ = **`r prms$genome$name`**; _Maximum mismatches_ = **`r prms$match$mismatch`**; _Allow INDEL_ = **`r prms$match$indel`**)
</div>
...
`r OrderFigure()` Number of hits in the reference genome matching each primer sequence, while allowing up to `r prms$match$mismatch` mismatches. Locations of hits are split according to whether they are on or off targeted regions. Color represents number of mismatched bases. 
  ...
  > [# of hits by primers and by mismatches](table/mismatch_count.html)
...
> [# of hits by primers and by mismatches](table/alignment_count_primer.html)
</div>
...
> [# of reads by primer hits and mismatches](table/alignment_summary_read.html)
</div>
